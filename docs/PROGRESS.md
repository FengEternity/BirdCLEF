...
### 2026-03-26（增量实验 A1 成功：LB 0.830，+0.041 + 计划修订）
...
- **V14 诊断分析（ANAL-005）**：
  - 训练声景 AUC 飙升到 0.995（+0.312），但 LB 仅 0.830 → 声景间泛化差距 0.165
  - 零样本物种从完全失效（pred ~0）变为高置信检测（pred ~0.85）
  - 瓶颈转移：从"域偏移"→"声景站点多样性"
- **实验路线图修订（DES-002 V2）**：
  - 伪标签（D2）提前到第二优先级（直接攻击声景多样性瓶颈）
  - A2（噪声增强）降级跳过（训练声景已 0.995）
  - 新路线：A1 ✓ → B1（10s 训练）→ D2（伪标签）→ B2（GeM）→ D1（5-fold）
